...
class View
{
	...
	*/
	...
	private function template_html_loop($f){
		...
		if(isset($a['nextpage'])){$nextpage=trim($a['nextpage'],"'");}else{$nextpage='page';}
		if(isset($a['sql'])){$sql=trim($a['sql'],"'");}else{$sql='';}
		if(isset($a['nextcache'])){$nextcache=trim($a['nextcache'],"'");}else{$nextcache=false;}
		if(isset($a['nextcachetime'])){$nextcachetime=trim($a['nextcachetime'],"'");}else{$nextcachetime=30*60;}
		if(isset($a['orderby'])){
			$order=$a['orderby'];
		...
		unset($a['table']);unset($a['orderby']);unset($a['limit']);unset($a['as']);unset($a['like']);unset($a['notlike']);unset($a['fields']);unset($a['isall']);unset($a['notin']);unset($a['notempty']);unset($a['empty']);unset($a['day']);unset($a['in']);unset($a['sql']);unset($a['nextpage']);unset($a['nextcache']);unset($a['nextcachetime']);
		$pages='';
		$w = ' 1=1 ';
		$ispage=false;
		if($nextpage!='page'){
			if(stripos($nextpage,'$')!==false){
				$nextpage = "'.$nextpage.'";
			}
			$pagenum = "\$pagenum = (int)\$_REQUEST['".$nextpage."'] ? (int)\$_REQUEST['".$nextpage."']  : 1; ";
		}else{
		...
		foreach($a as $k=>$v){
			...
			if($k=='ispage'){
				...
				$w.=" and nextattr like \'%,1,%\' ";
			}else if($k=='ishot'){
				$w.=" and nextattr like \'%,2,%\' ";
			}else if($k=='istuijian'){
				$w.=" and nextattr like \'%,3,%\' ";
			}else if($k=='nextattr'){
				if(strpos($v,',')!==false){
					$s = explode(',',$v);
					$s_sql = [];
					foreach($s as $ss){
						$s_sql[]=" nextattr like \'%,".$ss.",%\'  ";
					}
					$w.=" and ( ".implode('or',$s_sql)." ) ";
				}else{
					$w.=" and nextattr like \'%,".$v.",%\'";
				}
		...
		if($ispage){
			...
			if($nextcache){
				...
				if(!$".$as."_data){
					...
					setCache(\$cachestr,$".$as."_data,$nextcachetime);
				}";
			}else{
				$txt .= "
				$".$as."_data = M(\$".$as."_table)->findAll(\$".$as."_w,\$".$as."_order,\$".$as."_fields,\$".$as."_limit);";
