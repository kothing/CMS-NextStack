    ...
    public function route()
    {
		...
		if( !isset($webconfig['closesession']) || (isset($webconfig['closesession']) && $webconfig['closesession']==0) || APP_HOME=='app/admin'){
			...
			if(open_redis_session){
				...
				$session = new \NextSession(array('save_path'=>Session_Path,'life_time'=>SessionTime));
				session_set_save_handler($session,true);
				if (!isset($_COOKIE['PHPSESSID'])) {
		...
		if (!class_exists($controller) || !method_exists($controller, $actionName)) {
            ...
            if($controllerName=='Home' && $actionName=='next'){
            	if(method_exists($app_home.'\\plugins\\HomeController', 'next')){
    ...
    public function setDbConfig()
    {
        if ($this->config['db']) {
			...
			if(DB_HOST=='' || DB_NAME=='' || DB_USER=='' || DB_PASS==''){
				exit('<head><title>NextStack</title><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head><body><div style="text-align: center; line-height: 4rem;">数据库无法链接，如果您是第一次使用，请先执行<a href="/install/">安装程序</a></div></body>');
			}
...
if (strpos($url, '/index.php/'.ADMIN_MODEL) !== false || (defined('APP_HOME') && APP_HOME=='app/admin')) {
    ...
    // 定义项目模板文件目录
    defined('HOME_VIEW') or define('HOME_VIEW', 'template');

    defined('Tpl_template') or define('Tpl_template', 'tpl');

    // 定义项目控制器文件目录
    defined('HOME_CONTROLLER') or define('HOME_CONTROLLER', 'controller');
    ...
    //定义静态文件路径
    defined('Tpl_style') or define('Tpl_style', '/app/admin/template/tpl');
} else {
    //前台
    //定义项目目录
    ...
    //定义项目默认方法
    defined('DefaultAction') or define('DefaultAction', 'next');
