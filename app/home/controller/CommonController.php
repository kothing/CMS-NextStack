...
class CommonController extends Controller
{
				...
				if(strpos($_SESSION['member']['paction'],','.APP_CONTROLLER.',')!==false){
					...
					if(strpos($_SESSION['member']['paction'],','.$action.',')==false){
						...
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('您没有').'【'.$ac['name'].'】'.NEXTLANG('的权限！'),'url'=>U('Home/index')]);
						}
						Error(NEXTLANG('您没有').'【'.$ac['name'].'】'.NEXTLANG('的权限！'),U('Home/index'));
					}
				}
	...
	function multiuploads(){
		...
		$data = ['code'=>0,'urls'=>$response,'msg'=>NEXTLANG('上传成功！')];
		JsonReturn($data);
	...
	function uploads(){
		...
		if ($_FILES[$file]["error"] > 0){
		  $data['error'] =  "Error: ".NEXTLANG("上传错误！");
		  $data['code'] = 1000;
		}else{
		  ...
		  if(!$this->webconf['isopenhomeupload']){
			...
			if(move_uploaded_file($_FILES[$file]['tmp_name'],$filename)){
				...
				$data['error'] =  "Error: ".NEXTLANG("请检查目录")."[public/Home]".NEXTLANG("写入权限");
				$data['code'] = 1001;
			...
			switch($v['fieldtype']){
				...
				$l .= '<div class="form-control">
		            ...
		            if($data[$v['field']]){
		            	foreach(explode('||',$data[$v['field']]) as $s){
		            		if($s!=''){
                                $pic = explode('|',$s);
                                $l .= '<span><img src="'.$pic[0].'" height="100"  /><input name="'.$v['field'].'_urls[]" type="text" value="'.$pic[0].'"><input name="'.$v['field'].'_des[]" type="text" placeholder="'.NEXTLANG('文字描述').'"  value="'.$pic[1].'" ><button type="button" onclick="deleteImage_auto(this)">'.NEXTLANG('删除').'</button></span>';
					       ...
					       success: function(response){
					        if(response.code==0){
					          ...
					          for(var i=0;i<response["urls"].length;i++){
					          	result +=\'<span><img src="\' + response["urls"][i] + \'" height="100"  /><input name="'.$v['field'].'_urls[]" type="text" value="\' + response["urls"][i] + \'" ><input name="'.$v['field'].'_des[]" type="text" placeholder="'.NEXTLANG('文字描述').'"  value="" ><button type="button" onclick="deleteImage_auto(this)">'.NEXTLANG('删除').'</button></span>\';
					          }
				...
				$l .= '<div class="form-control">
		            ...
		            if($data[$v['field']]){
		            	foreach(explode('||',$data[$v['field']]) as $s){
		            		if($s!=''){
                                $pic = explode('|',$s);
                                $l .= '<span><input name="'.$v['field'].'_urls[]" type="text" value="'.$pic[0].'"><input name="'.$v['field'].'_des[]" type="text" placeholder="'.NEXTLANG('文字描述').'"  value="'.$pic[1].'" ><button type="button" onclick="deleteImage_auto(this)">'.NEXTLANG('删除').'</button></span>';
					    ...
					    $.ajax({
					       ...
					       success: function(response){
					        if(response.code==0){
					          var result = "";
					          for(var i=0;i<response["urls"].length;i++){
                                 result +=\'<span><input name="'.$v['field'].'_urls[]" type="text" value="\' + response["urls"][i] + \'" ><input name="'.$v['field'].'_des[]" type="text" placeholder="'.NEXTLANG('文字描述').'"  value="" ><button type="button" onclick="deleteImage_auto(this)">'.NEXTLANG('删除').'</button></span>\';
					        }
					          $(".view_img_'.$v['field'].'").append(result);
					        }else{
					          alert(response.error);
				...
				$l .= '<div class="form-control">
		            ...
		            if($data[$v['field']]){
		            	foreach(explode('||',$data[$v['field']]) as $s){
		            		if($s){
								$l .= '<div class="form-control"><input name="'.$v['field'].'[]" type="text" value="'.$s.'"><button type="button" class="'.$v['field'].'_del">'.NEXTLANG('删除').'</button></div>';
				...
				<div class="form-control">
		            <label ></label>
					<button type="button" class="layui-btn" id="'.$v['field'].'_add">'.NEXTLANG('新增').'</button>
					<label  class="fields_tips">'.$must.$v['tips'].'</label>
		        </div>
				<script>
				$(document).ready(function(){
					$("#'.$v['field'].'_add").click(function(){
						var html = \'<div class="form-control"><input type="text"  style="width:500px;" value="" name="'.$v['field'].'[]" autocomplete="off" ><button type="button" class="'.$v['field'].'_del" >'.NEXTLANG('删除').'</button></div>\';
