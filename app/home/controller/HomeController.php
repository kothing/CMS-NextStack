...
class HomeController extends CommonController
{
	...
	function next(){
		...
		if($res){
            ...
            $page = new Page($molds);
            $nextattr = $this->frparam('attr',1);
            if($nextattr){
                $nextattr_arr = explode('-',$nextattr);
                foreach ($nextattr_arr as $s){
                    $sql.=" and nextattr like '%,".$s.",%' ";
                }
	...
	function auto_url(){
		...
		if($id && $html){
			//详情页+html
			$res = M('classtype')->find(array('htmlurl'=>$html,'isclose'=>0));
			if(!$res){ $this->error(NEXTLANG('链接错误！'));}
			$isclass = false;
		}else if($id && $tid){
			//详情页
	...
	function search(){
		...
		if(in_array($molds,$tables) && $molds!=''){
			...
			if($word==''){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'data'=>'','msg'=>NEXTLANG('请输入关键词搜索！')]);
				}
				Error(NEXTLANG('请输入关键词搜索！'));
			...
			if($this->frparam('ajax')){
				JsonReturn(['code'=>1,'data'=>'','msg'=>NEXTLANG('搜索超出设定范围！')]);
			}
			Error(NEXTLANG('搜索超出设定范围！'));
		}
	...
	function searchAll(){
		...
		if($molds && is_array($molds)){
			...
			if(count($allow_table)==0){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'data'=>'','msg'=>NEXTLANG('您的搜索超出设定范围！')]);
				}
				Error(NEXTLANG('您的搜索超出设定范围！'));
			}
			...
			if(strpos($filepath,'/')!==false){
				...
				foreach($filepath as $vv){
					...
					if(!is_dir($create_dir)){
						...
						if(!$r){
							echo NEXTLANG('系统创建').' [ '.str_replace('/','\\',$create_dir).' ] '.NEXTLANG('目录失败!');exit;
						}
	...
	function rss(){
		...
		$xml = '<?xml version="1.0" encoding="utf-8"?>  
				...
				<channel>
					...
					<generator>Rss Powered By NextStack</generator>
					<image>
