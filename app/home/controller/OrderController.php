...
class OrderController extends CommonController
{
	function _init(){
		...
		if(!$this->islogin){
			...
			Error(NEXTLANG('您还未登录，请重新登录！'),U('Login/index'));
		}
		...
		if($this->frparam('go')){
			...
			if(!$order || !$w['orderno']){
				if($this->frparam('ajax')){
					...
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('订单号不存在或已被删除！'),'url'=>$return_url]);
					
				}
				Error(NEXTLANG('订单号不存在或已被删除！'),$return_url);
			}
			//购物订单
			if($order['ptype']==1){
				if($w['receive_username']=='' || $w['receive_tel']=='' || $w['receive_address']==''){
					if($this->frparam('ajax')){
						...
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('收件人、手机号和收货地址不能为空！'),'url'=>$return_url]);
						
					}
					
					Error(NEXTLANG('收件人、手机号和收货地址不能为空！'),$return_url);
				...
				if($this->webconf['paytype']==0){
					Error(NEXTLANG('未开启在线支付！'),$return_url);
				}
			}
			...
			if($this->webconf['isopenemail']==1 && $order['ptype']==1){
				if($this->webconf['email_server'] && $this->webconf['email_port'] &&  $this->webconf['send_email'] &&  $this->webconf['send_pass']){
					$title = NEXTLANG('您的订单提交成功通知').'-'.$this->webconf['web_name'];
					if($this->webconf['tj_msg']!=''){
						$body = str_replace('{xxx}',$w['receive_username'],$this->webconf['tj_msg']);
					}else{
						$body = NEXTLANG('尊敬的').$w['receive_username'].NEXTLANG('我们已经收到您的订单，我们会尽快给你发货，请密切关注您的邮箱以获得订单的最新消息，谢谢合作！');
					}
					
					$body.='<br/>'.NEXTLANG('订单详细信息如下').'：<br/>';
					$body.='<table style="min-width:500px">
					<tr><th width="20%">'.NEXTLANG('主图').'</th><th width="20%">'.NEXTLANG('商品').'</th><th width="20%">'.NEXTLANG('价格').'</th><th width="20%">'.NEXTLANG('购买数量').'</th><th width="20%">'.NEXTLANG('总价').'</th></tr>';
					...
					foreach(explode('||',$order['body']) as $v){
						if($v!=''){
							...
							if($d[0]!=''){
								...
								$body.='<tr><td width="20%"><img width="200px" src="'.get_domain().$product['litpic'].'" /></td><td width="20%">'.$product['title'].'</td><td width="20%">'.NEXTLANG('￥').$d[3].NEXTLANG('元').'</td><td width="20%">'.$d[2].'</td><td width="20%">'.NEXTLANG('￥').($d[3]*$d[2]).NEXTLANG('元').'</td></tr>';
					...
					$body.='<tr><td>'.NEXTLANG('折扣').'：</td><td colspan="4">'.NEXTLANG('￥').$order['discount'].NEXTLANG('元').'</td></tr><tr><td>'.NEXTLANG('运费').'：</td><td colspan="4">'.NEXTLANG('￥').$order['yunfei'].NEXTLANG('元').'</td></tr><tr><td>'.NEXTLANG('合计').'：</td><td colspan="4">￥'.$order['price'].'元</td></tr></table><br/>';
					$body.=NEXTLANG('收件地址').'：'.$w['receive_address'].' '.NEXTLANG('联系电话').'：'.$w['receive_tel'];
			...
			if($this->webconf['paytype']==0 && $order['ptype']==1){
				...
				//更新订单状态，提示收到提交订单
				M('orders')->update(['id'=>$order['id']],['isshow'=>4,'paytype'=>NEXTLANG('线下支付')]);
				...
				$task['type'] = 'rechange';
				$task['addtime'] = time();
				$task['body'] = NEXTLANG('您的订单').'-'.$order['orderno'].NEXTLANG('已经提交，我们会尽快给您发货！');
				$task['url'] = U('user/orderdetails',['orderno'=>$order['orderno']]);
				M('task')->add($task);

				if($this->frparam('ajax')){
				
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('我们已经收到您的订单，我们会尽快给你发货，请密切关注您的邮箱以获得订单的最新消息，谢谢合作！'),'url'=>U('user/orders')]);
				}
				
				Success(NEXTLANG('我们已经收到您的订单，我们会尽快给你发货，请密切关注您的邮箱以获得订单的最新消息，谢谢合作！'),U('User/orders'));
				...
				if($order['ispay']==1){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('订单已支付！'),'url'=>$return_url]);
					}
					
					Error(NEXTLANG('订单已支付！'),$return_url);
				}
				...
				$task['type'] = 'rechange';
				$task['addtime'] = time();
				$task['body'] = NEXTLANG('您的订单').'-'.$order['orderno'].NEXTLANG('已经提交，请尽快支付！');
				$task['url'] = U('user/orderdetails',['orderno'=>$order['orderno']]);
				M('task')->add($task);
				if(isMobile()){
					//手机端
					if(isWeixin()){
						...
						M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('支付宝H5支付')]);
						...
						$orderName = NEXTLANG('支付订单').'-'.$order['orderno'];    //订单标题
					...
					M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('电脑支付宝支付')]);
					/*** 请填写以下配置信息 ***/
					extendFile('pay/alipay/AlipayService.php');
					$appid = $this->webconf['alipay_partner'];  //https://open.alipay.com 账户中心->密钥管理->开放平台密钥，填写添加了电脑网站支付的应用的APPID
					$returnUrl = U('Mypay/alipay_return_pay');     //付款成功后的同步回调地址
				...
				if(isMobile()){
					...
					if(isWeixin()){
						//微信内
						$order['paytype'] = 'wxpay';
						M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('微信内支付')]);
						...
						M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('微信H5支付')]);
					...
					M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('微信扫码支付')]);
				...
				if($this->webconf['isopenqianbao']!=1){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('未开启钱包支付！'),'url'=>$return_url]);
					}
					Error(NEXTLANG('未开启钱包支付！'),$return_url);
				}
				//钱包支付
				...
				if($money<$allmoney){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('钱包金额不足，请充值！'),'url'=>$return_url]);
					}
					
					Error(NEXTLANG('钱包金额不足，请充值！'),$return_url);
				}

				$money_x = $money-$allmoney;
				$paytime = time();
				M('orders')->update(['id'=>$order['id']],['ispay'=>1,'isshow'=>2,'paytime'=>$paytime,'paytype'=>NEXTLANG('钱包支付')]);
				M('member')->update(['id'=>$order['userid']],['money'=>$money_x]);
				$ww['userid'] = $order['userid'];
				$ww['amount'] = $allmoney;
				$ww['money'] = $order['price'];
				$ww['type'] = 2;
				$ww['msg'] = NEXTLANG('钱包支付');
				...
				if($this->webconf['isopenjifen']!=1){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('未开启积分支付！'),'url'=>$return_url]);
					}
					Error(NEXTLANG('未开启积分支付！'),$return_url);
				...
				if($jifen<$order['jifen']){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('积分不足，请充值！'),'url'=>$return_url]);
					}
					
					Error(NEXTLANG('积分不足，请充值！'),$return_url);
				}
				$money_x = $jifen-$allmoney;
				$paytime = time();
				M('orders')->update(['id'=>$order['id']],['ispay'=>1,'isshow'=>2,'paytime'=>$paytime,'paytype'=>NEXTLANG('积分兑换')]);
				...
				M('orders')->update(['id'=>$order['id']],['paytype'=>NEXTLANG('支付宝当面付')]);
				/*** 请填写以下配置信息 ***/
				extendFile('pay/alipay/AlipayService.php');
				$appid = $this->webconf['alipay_partner'];  //https://open.alipay.com 账户中心->密钥管理->开放平台密钥，填写添加了电脑网站支付的应用的APPID
				$returnUrl = U('Mypay/alipay_return_pay');     //付款成功后的同步回调地址
				...
				//进入第三方支付内
				$order['paytype'] = $this->frparam('payname',1,NEXTLANG('其他平台支付'));
