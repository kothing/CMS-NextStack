		...
		if($_POST){
			...
			if($w['tel']){
				if(!preg_match("/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[1-9])\\d{8}$/",$w['tel'])){
					...
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('您的手机号格式不正确！')]);
					}
					Error(NEXTLANG('您的手机号格式不正确！'));
				}
			...
			if($res){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('提交成功！我们会尽快回复您！'),'url'=>get_domain()]);
				}
				Success(NEXTLANG('提交成功！我们会尽快回复您！'),get_domain());
			}else{
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('提交失败，请重试！')]);
				}
				//$this->error('提交失败，请重试！');
				Error(NEXTLANG('提交失败，请重试！'));
			}
