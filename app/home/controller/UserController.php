...
class UserController extends CommonController
{
	function _init(){
		...
		if(!M('molds')->find(['biaoshi'=>'member','isopen'=>1])){
			if($this->frparam('ajax')){
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('会员中心已关闭！')]);
			}
			Error(NEXTLANG('会员中心已关闭！'));
			exit;
		}
		parent::_init();
	...
	function checklogin(){
		if(!$this->islogin){
			if($this->frparam('ajax')){
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('您还未登录，请重新登录！')]);
			}
	...
	function userinfo(){
		...
		if($_POST){
            ...
            if(!isset($w['csrfkey']) || $w['csrfkey']!=$_SESSION['csrfkey']){
                if($this->frparam('ajax')){
                    JsonReturn(['code'=>1,'msg'=>NEXTLANG('非法操作！')]);
                }
                Error(NEXTLANG('非法操作！'));
            }
			...
			if($w['tel']!=''){
				if(preg_match("/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[0-9])\\d{8}$/",$w['tel'])){
					...
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('手机号码格式错误！')]);
					}
					Error(NEXTLANG('手机号码格式错误！'));
				...
				if($r){
					if($r['id']!=$this->member['id']){
						...
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('手机号已被注册！')]);
						}
						Error(NEXTLANG('手机号已被注册！'));
					}
				}
			}
			if($w['username']==''){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('账户不能为空！')]);
				}
				Error(NEXTLANG('账户不能为空！'));
			}
			if($w['pass']!=$w['repass'] && $w['pass']!=''){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('两次密码不同！')]);
				}
				Error(NEXTLANG('两次密码不同！'));
			}
			if($w['email']){
				...
				if($r){
					if($r['id']!=$this->member['id']){
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('邮箱已被使用！')]);
						}
						Error(NEXTLANG('邮箱已被使用！'));
					}
				}
			}
			...
			if($r){
				if($r['id']!=$this->member['id']){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('昵称已被使用！')]);
					}
					Error(NEXTLANG('昵称已被使用！'));
				}
			}
			if($w['pass']!=''){
			...
			if($this->frparam('ajax')){
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('修改成功！')]);
			}
			Error(NEXTLANG('修改成功！'));
	...
	function payment(){
		...
		if($this->frparam('go') && $orderno && $order){
			if($order['isshow']!=1){
				...
				if($order['isshow']==3){
					$msg = NEXTLANG('订单已过期，不可支付！');
				}
				if($order['isshow']==2){
					$msg = NEXTLANG('订单已支付，请勿重复操作！');
				}
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>$msg]);
				}
	...
	function likesAction(){
		...
		if(!$tid || !$id){
			if($this->frparam('ajax')){
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('参数错误！'),'url'=>$_SESSION['return_url']]);
			}
			Error(NEXTLANG('参数错误！'));
		}
		if(!$this->islogin){
			...
			$lk = $tid.'-'.$id;
			$u = M('member')->find(['username'=>'nextcustomer']);
			if(!$u){
				$w = [];
				$w['username'] = 'nextcustomer';
				$r = M('member')->add($w);
				$u['id'] = $r;
				$u['likes'] = '';
			}
		...
		if(!$this->classtypedata[$tid]['molds']){
			if($this->frparam('ajax')){
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('栏目未绑定模型，无法喜欢！'),'url'=>$_SESSION['return_url']]);
			}
			Error(NEXTLANG('栏目未绑定模型，无法喜欢！'));
		}
		$molds = $this->classtypedata[$tid]['molds'];
		$isaward = 0;
		...
		if($res){
			...
			if($award){
				...
				$sql = " addtime>=".$start." and addtime<".$end." and userid=".$member_id." and type=3 and msg='".NEXTLANG('点赞奖励')."' ";
	...
	function likes(){
		...
		foreach($data as $k=>$v){
			if(isset($this->classtypedata[$v['tid']])){
				...
				$data[$k]['details'] = $vdata ? $vdata : ['title'=>NEXTLANG('已被删除')];
			}else{
				$data[$k]['classname'] = '[ '.NEXTLANG('已被删除 ').']';
				$data[$k]['url'] = '';
				$data[$k]['details'] = [];
			}
	...
	function likesdel(){
		...
		if($id){
			...
			Success(NEXTLANG('删除成功！'),U('user/likes'));
		}else{
			Error(NEXTLANG('参数错误！'));
		}
	}
	...
	function collectAction(){
		...
		if(!$this->classtypedata[$tid]['molds']){
			if($this->frparam('ajax')){
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('栏目未绑定模型，无法收藏！'),'url'=>$_SESSION['return_url']]);
			}
			Error(NEXTLANG('栏目未绑定模型，无法收藏！'));
		}
		...
		if($r){
			...
			if($isaward){
				...
				$sql = " addtime>=".$start." and addtime<".$end." and userid=".$member_id." and type=3 and msg='".NEXTLANG('收藏奖励')."' ";
				$all = M('buylog')->findAll($sql,null,'amount');
				$all_jifen = 0;
				if($all){
					foreach($all as $v){
	...
	function collectdel(){
        ...
        if($id){
            ...
            Success(NEXTLANG('删除成功！'),U('user/collect'));
        }else{
            Error(NEXTLANG('参数错误！'));
        }
	}
	...
	function addcart(){
		...
		if($product['stock_num']<$num){
			JsonReturn(['code'=>1,'msg'=>NEXTLANG('库存不足！')]);
		}
    ...
    function release(){
    	...
    	if($_POST){
			...
			if(!in_array($w['molds'],$release_table)){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('该模块不允许发布！')));
			}
									...
									foreach($mingan as $s){
										if(strpos($s,'{xxx}')!==false){
											...
											if(preg_match($pattern, $vv)){
												JsonReturn(array('code'=>1,'msg'=>NEXTLANG('添加失败，存在敏感词').' [ '.$s.' ]'));
											}
			...
			if(!$w['tid']){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('请选择分类！')]);
				}else{
					Error(NEXTLANG('请选择分类！'));
				}
				
			}
			if(!isset($this->classtypedata[$w['tid']])){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('分类错误！')]);
				}else{
					Error(NEXTLANG('分类错误！'));
				}
			...
			if($this->classtypedata[$w['tid']]['ishome']!=1){
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('该分类不允许发布！')]);
				}else{
					Error(NEXTLANG('该分类不允许发布！'));
				}
			}
			//检查权限
			if($this->classtypedata[$w['tid']]['gid']!=0){
				if($this->classtypedata[$w['tid']]['gid']>$this->member['gid']){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('您没有权限在该分类发布内容！')]);
					}else{
						Error(NEXTLANG('您没有权限在该分类发布内容！'));
					}
				}
						...
						if($data[$v['field']]==''){
							if(in_array($v['fieldtype'],array(6,10))){
								if($data[$v['field'].'_urls']==''){
									...
									if($this->frparam('ajax')){
										JsonReturn(['code'=>1,'msg'=>$v['fieldname'].NEXTLANG('不能为空！')]);
									}else{
										Error($v['fieldname'].NEXTLANG('不能为空！'));
									}
								}
							}else{
								if($this->frparam('ajax')){
			...
			switch($w['molds']){
				case 'article':
					if(!$w['body']){
						...
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('内容不能为空！')]);
						}else{
							Error(NEXTLANG('内容不能为空！'));
						}
					}
					if(!$w['title']){
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('标题不能为空！')]);
						}else{
							Error(NEXTLANG('标题不能为空！'));
						}
					}
					$w['seo_title'] = $w['title'];
				...
				case 'product':
					...
					if(!$w['stock_num']){
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('库存不能为0！')]);
						}else{
							Error(NEXTLANG('库存不能为0！'));
						}
					}
					$w['seo_title'] = $w['title'];
				break;
			...
			if($this->frparam('id')){
				...
				if(!$a){ 
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('未修改内容，不能提交！')]);
					}else{
						Error(NEXTLANG('未修改内容，不能提交！'));
					}
				}
				if($this->frparam('ajax')){
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('修改成功！'),'url'=>U('user/posts',['molds'=>$w['molds']])]);
				}else{
					Success(NEXTLANG('修改成功！'),U('user/posts',['molds'=>$w['molds']]));
				}
				...
				if(!$a){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('发布失败，请重试！')]);
					}else{
						Error(NEXTLANG('发布失败，请重试！'));
					}
				}
				if($this->frparam('ajax')){
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('发布成功！'),'url'=>U('user/posts',['molds'=>$w['molds']])]);
				}else{
					Success(NEXTLANG('发布成功！'),U('user/posts',['molds'=>$w['molds']]));
				}
	...
	function del(){
		...
		$molds = $this->frparam('molds',1,'article');
		$id = $this->frparam('id');
		if(!$id){ Error(NEXTLANG('链接错误！'));}
		$res = M($molds)->find(['id'=>$id,'member_id'=>$this->member['id']]);
		if(!$res){ Error(NEXTLANG('未找到您要的文章！'));}
		$r = M($molds)->delete(['id'=>$id]);
		if($r){
			Success(NEXTLANG('删除成功！'),U('user/posts',['molds'=>$molds]));
		}else{
			Error(NEXTLANG('删除失败！'));
		}
	...
	function uploads(){
		...
		if ($_FILES[$file]["error"] > 0){
		  ...
		  if(!$this->webconf['isopenhomeupload']){
			  $data['error'] =  "Error: ".NEXTLANG("已关闭前台上传文件功能");
			  $data['code'] = 1004;
			  JsonReturn($data);
		  }
			...
			if($fileSize!=0 && ($_FILES[$file]["size"]/1024)>$fileSize){
				$data['error'] =  "Error: ".NEXTLANG("文件大小超过网站内部限制！");
				$data['code'] = 1003;
				JsonReturn($data);
			}
			...
			if(move_uploaded_file($_FILES[$file]['tmp_name'],$filename)){
				...
				$data['error'] =  "Error: ".NEXTLANG("请检查目录")."[".$home_save_path."]".NEXTLANG("写入权限");
				$data['code'] = 1001;
	...
	function follow(){
		...
		if($uid){
			...
			if(strpos($follow,','.$uid.',')!==false){
				Error(NEXTLANG('您已经关注了该用户！'));
			}
			if($uid==$member_id){
				Error(NEXTLANG('您不能关注自己！'));
			}
			//拼接方式 [ , ]
			if($follow==''){
				$follow = ','.$uid.',';
			...
			if($this->webconf['follow_award_open']==1){
				...
				if($member_id!=0 && $award>0){
					$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>$molds,'aid'=>$id,'msg'=>NEXTLANG('关注奖励')]);
					if(!$rr){
						$start = mktime(0, 0, 0, date('m'), date('d'), date('Y'));
						$end = mktime(23, 59, 59, date('m'), date('d'), date('Y'));
						$sql = " addtime>=".$start." and addtime<".$end." and userid=".$member_id." and type=3 and msg='".NEXTLANG('关注奖励')."' ";
						$all = M('buylog')->findAll($sql,null,'amount');
						$all_jifen = 0;
						if($all){
							foreach($all as $v){
			...
			Success(NEXTLANG('关注成功！'),U('user/follow'));
	...
	function nofollow(){
		...
		if($uid){
			...
			Success(NEXTLANG('取关成功！'),U('user/follow'));
	...
	*/
	...
	function allread(){
		...
		JsonReturn(['code'=>0,'msg'=>NEXTLANG('操作成功！')]);
    ...
    function notifydel(){
    	...
    	if($r){
    		Success(NEXTLANG('删除成功！'),U('user/notify'));
    	}else{
    		Error(NEXTLANG('删除失败！'));
    	}
    }
    ...
    function active(){
    	...
    	if($username || $uid){
    		...
    		if($this->frparam('ajax')){
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('链接错误！'),'data'=>'']);
			}
			Error(NEXTLANG('链接错误！'));
    	}

		if(!$this->user){
			Error(NEXTLANG('用户未找到！'));
		}
		//统计评论数
    ...
    function setmsg(){
    	...
    	if($_POST){
    		...
    		if($this->frparam('ajax')){
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('设置成功'),'data'=>'']);
			}
			Success(NEXTLANG('设置成功！'),U('user/setmsg'));
    	}
    ...
    function buy(){
    	...
    	if($_POST){
    		if($this->webconf['paytype']==0){
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('未开启在线支付！'),'data'=>'']);
			}
    		$money = $this->frparam('allmoney',3);
    		$number = $this->frparam('number');
    		if(!$money || !$number){
    			JsonReturn(['code'=>1,'msg'=>NEXTLANG('参数错误！'),'data'=>'']);
    		}
    		$w['jifen'] = $number;
    		$w['price'] = $money;
    		$w['orderno'] = 'No'.date('YmdHis');
	...
	function preview(){
    	...
    	if(!$id){
			$this->error(NEXTLANG('缺少ID！'));
		}
		if(!$tid){
			$this->error(NEXTLANG('缺少栏目ID！'));
		}
		...
		$positions='<a href="'.get_domain().'">'.NEXTLANG('首页').'</a>';
		...
		if($this->frparam('ajax') && $this->webconf['isajax'] ){
			...
			JsonReturn(['code'=>0,'next'=>$details,'prev'=>$aprev,'next'=>$anext]);
		}
