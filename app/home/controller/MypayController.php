	...
	**/
	...
	public function topay(){
		...
		if($this->islogin){
			...
			$w['username'] = $this->frparam('username',1,NEXTLANG('匿名'));
		}
		...
		$w['tid'] = $tid;
		if(!$id){
			Error(NEXTLANG('缺少id参数！'));
		}
		if(!$tid){
			Error(NEXTLANG('缺少tid参数！'));
		}
		...
		if(!$product){
			Error(NEXTLANG('未找到商品或者已下架！'));
		}
		if(!array_key_exists('price',$product)){
			Error(NEXTLANG('该模块缺少price价格参数！'));
		}
		...
		if($res){
			...
			switch($ptype){
				...
				case 3:
					...
					if(!$this->islogin){
						if($this->frparam('ajax')){
							JsonReturn(['code'=>1,'msg'=>NEXTLANG('您未登录，无法支付！'),'url'=>$return_url]);
						}
						Error(NEXTLANG('您未登录，无法支付！'),$return_url);
					}
					//钱包支付
	...
	public function alipay_check_order(){
		...
		if($result['alipay_trade_query_response']['code']!='10000'){
			...
			switch($result['alipay_trade_query_response']['trade_status']){
				case 'WAIT_BUYER_PAY':
					$msg = NEXTLANG('交易创建，等待买家付款');
					break;
				case 'TRADE_CLOSED':
					$msg = NEXTLANG('未付款交易超时关闭，或支付完成后全额退款');
					break;
				case 'TRADE_SUCCESS':
					$msg = NEXTLANG('支付成功');
					$code = 0;
