	...
	function index(){
		...
		if(!M('molds')->find(['isopen'=>1,'biaoshi'=>'comment'])){
			if($this->frparam('ajax')){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('评论模块未开启！')));
			}
			Error(NEXTLANG('评论模块未开启！'));
		}
		...
		if($this->frparam('go',0,false,"POST")){
			if($this->islogin){
				...
				if($_SESSION['message_num']>10 && ($_SESSION['message_time']+10*60)>time()){
					if($this->frparam('ajax')){
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('您的操作过于频繁，请十分钟后再试~')));
					}
					
					Error(NEXTLANG('您的操作过于频繁，请十分钟后再试~'));
				}
				...
				if(!$w['body']){
					if($this->frparam('ajax')){
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('评论内容不能为空！')));
					}
					Error(NEXTLANG('评论内容不能为空！'));
				}
				if(!$w['tid']){
					if($this->frparam('ajax')){
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('请提交栏目ID')));
					}
					Error(NEXTLANG('栏目ID不能为空！'));
				}
				//是否主帖子
				if($w['zid']){
				...
				if($n){
					...
					if($this->webconf['comment_award_open']==1 && $w['tid']!=0 && $w['aid']!=0){
						...
						if($member_id!=0 && $award>0 && $member_id!=$this->member['id']){
							$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>$molds,'aid'=>$id,'msg'=>NEXTLANG('评论奖励')]);
							if(!$rr){
								$start = mktime(0, 0, 0, date('m'), date('d'), date('Y'));
								$end = mktime(23, 59, 59, date('m'), date('d'), date('Y'));

								$sql = " addtime>=".$start." and addtime<".$end." and userid=".$member_id." and type=3 and msg='".NEXTLANG("评论奖励")."' ";
					...
					if($this->frparam('ajax')){
						JsonReturn(array('code'=>0,'msg'=>NEXTLANG('评价成功！'),'url'=>$url));
					}
					Success(NEXTLANG('评价成功！'),$url);
				}
				...
				if($this->frparam('ajax')){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('您未登录，请重新登录~'),'url'=>U('login/index')));
				}
