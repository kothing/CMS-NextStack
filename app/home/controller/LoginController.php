...
class LoginController extends CommonController
{
		...
		if($_POST){
			...
			if(!isset($this->webconf['closehomevercode']) || $this->webconf['closehomevercode']!=1){
				...
				if(!$vercode || md5(md5($vercode))!=$_SESSION['login_vercode']){
					$xdata = array('code'=>1,'msg'=>NEXTLANG('验证码错误！'));
					if($this->frparam('ajax')){
						JsonReturn($xdata);
					}
					Error(NEXTLANG('验证码错误！'));
				}
			}
			$_SESSION['login_vercode'] = getRandChar(32);
			if($data['username']=='' || $data['password']==''){
				$xdata = array('code'=>1,'msg'=>NEXTLANG('账户密码不能为空！'));
				if($this->frparam('ajax')){
					JsonReturn($xdata);
				}
				Error(NEXTLANG('账户密码不能为空！'));
			}
			...
			if($res || $res1 || $res2){
				...
				if($res['isshow']!=1){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('您的账户已被冻结！'),'url'=>$_SESSION['return_url']]);
					}
					Error(NEXTLANG('您的账户已被冻结！'));
				}

				$group = M('member_group')->find(array('id'=>$res['gid']));
				if(!$group){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('未找到您所在分组，请联系管理员处理！'),'url'=>$_SESSION['return_url']]);
				}
				unset($group['id']);
				//检测分组权限
				if($group['isagree']!=1){
					if($this->frparam('ajax')){
						JsonReturn(['code'=>1,'msg'=>NEXTLANG('您所在的分组被限制登录！'),'url'=>$_SESSION['return_url']]);
					}
					Error(NEXTLANG('您所在的分组被限制登录！'));
				}
                ...
                if($this->webconf['login_award_open']==1){
                	...
                	if($awoard>0){
                		...
                		$sql = " msg = '".NEXTLANG('登录奖励')."' and addtime>=".$start." and addtime<".$end." and userid=".$_SESSION['member']['id'];
				...
				//兼容ajax登录
				if($this->frparam('ajax')){
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('登录成功！'),'url'=>$_SESSION['return_url']]);
				}
				Redirect($_SESSION['return_url']);
			}else{
				if($this->frparam('ajax')){
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('账户密码错误！'),'url'=>$_SESSION['return_url']]);
				}
				Error(NEXTLANG('账户密码错误！'));
			}
	  ...
	  if($_POST){
		  ...
		  if($this->webconf['onlyinvite']==1){
			  if(!M('member')->find(['id'=>$this->frparam('pid'),'isshow'=>1])){
				    $xdata = array('code'=>1,'msg'=>NEXTLANG('您的邀请链接不合法！'));
				    if($this->frparam('ajax')){
						JsonReturn($xdata);
					}
					Error(NEXTLANG('您的邀请链接不合法！'));
			  }
		  ...
		  if($w['password']!=$w['repassword']){
				$xdata = array('code'=>1,'msg'=>NEXTLANG('两次密码不同！'));
				if($this->frparam('ajax')){
					JsonReturn($xdata);
				}
          ...
          if($w['tel']){
              if (preg_match("/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[0-9])\\d{8}$/", $w['tel'])) {
                  ...
                  if ($this->frparam('ajax')) {
                      JsonReturn(['code' => 1, 'msg' => NEXTLANG('手机号格式不正确！')]);
                  }
                  Error(NEXTLANG('手机号格式不正确！'));
              }
          }
		$w['regtime'] = time();
		//检查邮箱
		if($w['email']){
			if(M('member')->find(['email'=>$w['email']])){
				$xdata = array('code'=>1,'msg'=>NEXTLANG('您的邮箱已注册！'));
				if($this->frparam('ajax')){
					JsonReturn($xdata);
				}
				Error(NEXTLANG('您的邮箱已注册！'));
			}
		}
		//检查是否已被注册
		if(M('member')->find(['tel'=>$w['tel']])){
			$xdata = array('code'=>1,'msg'=>NEXTLANG('您的手机号码已注册！'));
			if($this->frparam('ajax')){
				JsonReturn($xdata);
			}
			Error(NEXTLANG('您的手机号码已注册！'));
		}
		...
		if($r){
			...
			if($this->frparam('autologin')){
				...
				if($member_group['isagree']!=1){
					$xdata = array('code'=>1,'msg'=>NEXTLANG('注册成功，等待审核！'),'url'=>U('login/index'));
				}else{
					unset($member['pass']);
					unset($member_group['id']);
					$_SESSION['member'] = array_merge($member,$member_group);
					$xdata = array('code'=>0,'msg'=>NEXTLANG('注册成功！'),'url'=>U('user/index'));
				}
				if($this->frparam('ajax')){
					JsonReturn($xdata);
				}
				...
				$xdata = array('code'=>0,'msg'=>NEXTLANG('注册成功！'),'url'=>U('login/index'));
				if($this->frparam('ajax')){
					JsonReturn($xdata);
				}
				Success(NEXTLANG('注册成功！'),U('login/index'));
			}
			
		}else{
			$xdata = array('code'=>1,'msg'=>NEXTLANG('注册失败，请重试~'));
			if($this->frparam('ajax')){
				JsonReturn($xdata);
			}
			Error(NEXTLANG('注册失败，请重试~'));
		}
  ...
  function forget(){
	  if($_POST && !isset($_POST['reset'])){
		  ...
		  $vercode = strtolower($this->frparam('vercode',1));
		  if(!$email){
			  Error(NEXTLANG('请输入账号和邮箱！'));
		  }
		  if($_SESSION['forget_code']!=md5(md5($vercode))){
			  Error(NEXTLANG('图形验证码错误！'));
		  }
		  ...
		  if($user){
			  ...
			  if($this->webconf['email_server'] && $this->webconf['email_port'] &&  $this->webconf['send_email'] &&  $this->webconf['send_pass']){
				$title = NEXTLANG('找回密码').'-'.$this->webconf['web_name'];
				$url = U('login/forget').'?token='.$w['token'].'&email='.$email;
				$body = NEXTLANG('您的账号正在进行找回密码操作，如果确定是本人操作，请在10分钟内点击').'<a href="'.$url.'" target="_blank">['.NEXTLANG('立即找回密码').']</a>，'.NEXTLANG('过期失效！');
				...
				if($_SESSION['forget_num']>5 && ($_SESSION['forget_time']+10*60)>time()){
					...
					if($this->frparam('ajax')){
						JsonReturn(['code'=>0,'msg'=>NEXTLANG('您操作过于频繁，请10分钟后再尝试！')]);
					}
					Error(NEXTLANG('您操作过于频繁，请10分钟后再尝试！'));
				}

				Success(NEXTLANG('找回密码邮件已发送，请到您的邮箱查看！'),get_domain());
				 
				
			 }else{
				 Error(NEXTLANG('邮箱服务器未配置，无法发送邮件，请联系管理员找回密码！'));
			 }
			  
		  }else{
			   Error(NEXTLANG('输入的信息有误！'));
		  }
	  }
	  if(!isset($_POST['reset']) && $this->frparam('token',1) && $this->frparam('email',1)){
		  ...
		  if($this->frparam('token',1)!='' && $this->frparam('email',1)!=''){
			  ...
			  if($user){
				  ...
				  if($t>10){
					  Error(NEXTLANG('token已失效！'),U('login/forget'));
				  }
				  $this->user = $user;
	  ...
	  if($_POST && isset($_POST['reset'])){
		  ...
		  if($w['token']!='' && $w['username']!='' && $pass!=''){
			 ...
			 if(!$user){
				 Error(NEXTLANG('参数错误！'),U('login/forget'));
			 }
			 $pass = md5(md5($pass).md5($pass));
			 if(M('member')->update(['id'=>$user['id']],['pass'=>$pass])){
				 Success(NEXTLANG('密码重置成功,请重新登录！'),get_domain());
				 
			 }else{
				 Error(NEXTLANG('新密码不能与旧密码相同！'));
			 }
  ...
  function loginout(){
      ...
      Error(NEXTLANG('安全退出~'),get_domain());
  }
