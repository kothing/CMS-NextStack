...
class ScreenController extends CommonController
{
	function index(){
		...
		//检测三个参数是否存在
		if(!$this->frparam('molds',1) || !$this->frparam('tid') || !$this->frparam('next_screen',1)){
			$this->error(NEXTLANG('参数错误！'));
		}
		if(!M('molds')->find(['biaoshi'=>$this->frparam('molds',1)])){
			$this->error(NEXTLANG('非法参数！'));
		}
		...
		//检测是否有page分页参数
		$this->frpage = 1;
		if(strpos($this->frparam('next_screen',1),'page')!==false){
			$next_screen_arr = explode('-page-',$this->frparam('next_screen',1));
			$next_screen = explode('-',$next_screen_arr[0]);
			$this->frpage = (int)$next_screen_arr[1];
		}else{
			$next_screen = explode('-',$this->frparam('next_screen',1));
		}
		...
		$screen = array_combine($next_screen_key,$next_screen_value);
