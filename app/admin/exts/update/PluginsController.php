...
class PluginsController extends Controller {
	...
	public  function install(){
		//下面是新增test表的SQL操作
		//检测版本号
		if(version_compare($this->webconf['web_version'],'0.0.1','>')){
			JsonReturn(['code'=>1,'msg'=>'您的软件系统版本为'.$this->webconf['web_version'].'，该插件仅支持0.0.1版本以下修复！']);
		}
		...
		M('sysconfig')->update(['field'=>'web_version'],['data'=>'0.0.1']);
	...
	//mysql版本
	private function NEXT_mysql(){
        
		/**
		 * 文件转移
		**/
		
		$dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/admin/tpl';
		$to = APP_PATH.APP_HOME.'/template/tpl';
		$this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/framework/common';
        $to = APP_PATH.'framework/common';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/framework/extend';
        $to = APP_PATH.'framework/extend';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/conf';
        $to = APP_PATH.'conf';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/admin/controller';
        $to = APP_PATH.APP_HOME.'/controller';
        $this->removeFile($dir,$to);
        
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/framework';
        $to = APP_PATH.'framework';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/framework/lib';
        $to = APP_PATH.'framework/lib';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/home';
        $to = APP_PATH.'app/home/controller';
        $this->removeFile($dir,$to);
        $dir = APP_PATH.APP_HOME.'/exts/update/file/mysql/static';
