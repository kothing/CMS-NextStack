...
class TemplateController extends CommonController
{
	...
	function desc(){
		...
		if(strpos($filepath,'.')!==false){
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('参数存在安全隐患！')));
		}
		if($filepath){
			//忽略Notice报错
		...
		JsonReturn(array('code'=>1,'msg'=>NEXTLANG('参数错误,必须携带插件ID！')));
	}
	...
	function update(){
		...
		if($template){
				...
				switch ($action) {
				    ...
				    case 'start-download':
				        ...
				        try {
				            ...
				            JsonReturn(['code'=>1,'msg'=>NEXTLANG('发生错误').'：'.$e->getMessage()]);
				        }
				    ...
				    case 'file-upzip':
				    	if (!file_exists($tmp_path)) {//先判断待解压的文件是否存在
						   JsonReturn(['code'=>1,'msg'=>NEXTLANG('下载缓存文件不存在！')]);
						}
				    ...
				    case 'template-install':
						...
						if(!count($tpl)){
							JsonReturn(array('code'=>1,'msg'=>NEXTLANG('请选择使用场景！')));
						}
				    	if(file_exists($dir.'/'.$template.'/install/TemplateController.php')){
							...
							if(!$step1){
								JsonReturn(array('code'=>1,'msg'=>NEXTLANG('执行插件安装程序失败！')));
							}
						}
						if(in_array('pc',$tpl)){
                        ...
                        setCache('templatelist',null);
						JsonReturn(array('code'=>0,'msg'=>NEXTLANG('安装成功！')));
				    	break;
					case 'backup':
						...
						$this->toBackup();
						JsonReturn(array('code'=>0,'msg'=>NEXTLANG('备份成功！')));
						break;
				    default:
				        # code...
				        break;
			...
			if($rr['code']==0){
				...
				exit(NEXTLANG('该插件暂无更新！'));
			}

			
		}
		JsonReturn(array('code'=>1,'msg'=>NEXTLANG('参数错误,请选择对应模板！')));
	}
	
	function upzip($filename,$path){
		//先判断待解压的文件是否存在
		if(!file_exists($filename)){
			JsonReturn(['code'=>1,'msg'=>$filename.NEXTLANG('文件不存在！')]);
		}
		$zip = new \ZipArchive;
		...
		if ($zip->open($filename) === TRUE) {//中文文件名要使用ANSI编码的文件格式
		  ...
		  $msg = NEXTLANG("解压完毕！本次解压花费")."：$thistime ".NEXTLANG("秒")."。";
		  return $msg;
		} else {
		  return NEXTLANG('解压失败！');
		...
		foreach ($data[$i] as $v){
			...
			if($countsql%($this->limit)==0){
				...
				if($filenum==0){
					...
					if(!$isok){
						JsonReturn(['code'=>1,'msg'=>'[ '.$this->backupPath.'/'.$backfile.'.php ] '.NEXTLANG('写入文件失败！')]);
					}
					$filenum++;
				}else{
					$isok = file_put_contents($this->backupPath.'/'.$backfile.'_v'.$filenum.'.php', $str);
					if(!$isok){
						JsonReturn(['code'=>1,'msg'=>'[ '.$this->backupPath.'/'.$backfile.'_v'.$filenum.'.php ]'.NEXTLANG(' 写入文件失败！')]);
					}
					$filenum++;
				}
				$str = $public_str;
