	...
	function index(){
		...
		if($this->frparam('ajax')){
			...
			foreach($pagelist as $k=>$v){
				...
				switch($v['isshow']){
					case 1:
					$v['new_isshow'] = '<span class="layui-badge">'.NEXTLANG('待付款').'</span>';
					break;
					case 2:
					$v['new_isshow'] = '<span class="layui-badge layui-bg-green">'.NEXTLANG('已付').'</span>';
					break;
					case 3:
					$v['new_isshow'] = '<span class="layui-badge layui-bg-orange">'.NEXTLANG('超时').'</span>';
					break;
					case 4:
					$v['new_isshow'] = '<span class="layui-badge">'.NEXTLANG('待审核待支付').'</span>';
					break;
					case 5:
					$v['new_isshow'] = '<span class="layui-badge layui-bg-black">'.NEXTLANG('已发货').'</span>';
					break;
					case 6:
					$v['new_isshow'] = '<span class="layui-badge layui-bg-gray">'.NEXTLANG('已废弃').'</span>';
					break;
					default:
					$v['new_isshow'] = '<span class="layui-badge layui-bg-blue">'.NEXTLANG('被删除').'</span>';
					break;
				...
				$v['new_ispay'] = $v['ispay']==1 ? '<span class="layui-badge layui-bg-green">'.NEXTLANG('已付').'</span>' : '<span class="layui-badge">'.NEXTLANG('未付').'</span>';
		...
		if($id){
			...
			if($_POST){
				...
				if($data['isshow']!=$isshow && $isshow==5){
					...
					if($this->webconf['email_server'] && $this->webconf['email_port'] &&  $this->webconf['send_email'] &&  $this->webconf['send_pass'] && $this->webconf['isopenemail']==1){
						...
						if($data['receive_email']!=''){
							$title = NEXTLANG('您的订单发货通知').'-'.$this->webconf['web_name'];
							if($this->webconf['send_msg']!=''){
								$body = str_replace('{xxx}',$data['receive_username'],$this->webconf['send_msg']);
							}else{
								$body = NEXTLANG('尊敬的').$data['receive_username'].'，'.NEXTLANG('您的订单已发货了，这几天请您留意一下快递，谢谢您的惠顾！期待再次为您服务！');
							}
							
							$body.='<br/>'.NEXTLANG('订单详细信息如下：').'<br/>';
							$body.='<table style="min-width:500px">
							<tr><th width="20%">'.NEXTLANG('主图').'</th><th width="20%">商品</th><th width="20%">'.NEXTLANG('价格').'</th><th width="20%">'.NEXTLANG('购买数量').'</th><th width="20%">'.NEXTLANG('总价').'</th></tr>';
							...
							$body.='<tr><td>'.NEXTLANG('折扣：').'</td><td colspan="4">￥'.$data['discount'].NEXTLANG('元').'</td></tr><tr><td>'.NEXTLANG('运费：').'</td><td colspan="4">'.NEXTLANG('￥').$data['yunfei'].NEXTLANG('元').'</td></tr><tr><td>'.NEXTLANG('合计：').'</td><td colspan="4">￥'.$data['price'].'元</td></tr></table><br/>';
							$body.=NEXTLANG('收件地址：').$data['receive_address'].' '.NEXTLANG('联系电话：').$data['receive_tel'];
				...
				$addtime = strtotime($this->frparam('addtime',1));
					...
					JsonReturn(['code'=>0,'msg'=>NEXTLANG('操作成功！'),'paytime'=>$paytime,'addtime'=>$addtime]);
	...
	function deleteAll(){
		...
		if($data!=''){
			...
			if(M('Orders')->delete('id in('.$data.')')){
				...
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('批量删除成功！')));
				
			}else{
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('批量操作失败！')));
			}
		}
	}
   ...
   function czlist(){
		...
		if($this->frparam('ajax')){
			...
			foreach($pagelist as $k=>$v){
				...
				$v['new_buytype'] = $v['buytype']=='money'?NEXTLANG('钱包'):NEXTLANG('积分');
				if($v['type']==1){
					$v['new_type'] = NEXTLANG('充值');
				}else if($v['type']==2){
					$v['new_type'] = NEXTLANG('兑换');
				}else{
					$v['new_type'] = NEXTLANG('奖励');
				}
 				$ajaxdata[]=$v;
   ...
   function chongzhi(){
   	  ...
   	  if($_POST){
   	  		...
   	  		if(!M('member')->find(['id'=>$w['userid']])){
   	  			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('该用户不存在！')));
   	  		}
   	  		...
   	  		if($w['amount']<=0){
   	  			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('充值数量不对！')));
   	  		}
   	  		if($w['buytype']=='money'){
   	  		...
   	  		if($r){
   	  			...
   	  			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('操作失败！')));
   	  		}
   	  }
