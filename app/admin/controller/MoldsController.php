...
class MoldsController extends CommonController
{
	...
	function addMolds(){
		if($this->frparam('go',1)==1){
			...
			if(!$data['name'] || !$data['biaoshi']){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('模块名和标识不能为空！')));
			}
			//检查是否已存在表
			if(M('molds')->find(['biaoshi'=>strtolower($data['biaoshi'])])){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('模型已添加不能重复添加！')));
			}
			$w = [];
			...
			if($this->frparam('hastable')){
				...
				if(in_array(DB_PREFIX.strtolower($data['biaoshi']),$ttable)){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('该表已存在！')));
				}
				$n = M('Molds')->add($w);
			...
			if($n){
				...
				`tid` int(11) DEFAULT 0 COMMENT '".NEXTLANG("所属栏目")."',
				`tids` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("副栏目")."',
				`title` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("标题")."',
				`litpic` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("缩略图")."',
				`keywords` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("关键词")."',
				`description` varchar(500) DEFAULT NULL COMMENT '".NEXTLANG("简介")."',
				`body` text DEFAULT NULL COMMENT '".NEXTLANG("内容")."',
				`molds` varchar(50) DEFAULT '".$data['biaoshi']."' COMMENT '".NEXTLANG("模型标识")."',
				`userid` int(11) DEFAULT 0 COMMENT '".NEXTLANG("发布管理员")."',
				`orders` int(11) DEFAULT 0 COMMENT '".NEXTLANG("排序")."',
				`member_id` int(11) DEFAULT 0 COMMENT '".NEXTLANG("前台用户")."',
				`comment_num` int(11) DEFAULT 0 COMMENT '".NEXTLANG("评论数")."',
				`htmlurl` varchar(100) DEFAULT NULL COMMENT '".NEXTLANG("栏目链接")."',
				`isshow` tinyint(1) DEFAULT 1 COMMENT '".NEXTLANG("是否显示")."',
				`target` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("外链")."',
				`ownurl` varchar(255) DEFAULT NULL COMMENT '".NEXTLANG("自定义URL")."',
				`nextattr` varchar(50) DEFAULT NULL COMMENT '".NEXTLANG("推荐属性")."',
				`hits` int(11) DEFAULT 0 COMMENT '".NEXTLANG("点击量")."',
				`zan` int(11) DEFAULT 0 COMMENT '".NEXTLANG("点赞数")."',
				`tags` varchar(255) DEFAULT NULL COMMENT 'TAG',
				`addtime` int(11) DEFAULT 0 COMMENT '".NEXTLANG("发布时间")."',
				PRIMARY 
				KEY  (`id`)
				...
				$w['fieldname'] = NEXTLANG('标题');
				$w['tips'] = NEXTLANG('默认为空');
				$w['fieldtype'] = 1;
				$w['fieldlong'] = 255;
				$w['ismust'] = 0;
				$w['isshow'] = 1;
				...
				$w['field'] = 'tid';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('所属栏目');
				$w['tips'] = NEXTLANG('选择栏目');
				$w['fieldtype'] = 17;
				$w['fieldlong'] = 11;
				$w['ismust'] = 0;
				$w['isshow'] = 1;
				...
				$w['field'] = 'tids';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('副栏目');
				$w['tips'] = NEXTLANG('绑定后可以在当前模型的其他栏目中显示');
				$w['fieldtype'] = 18;
				$w['fieldlong'] = 255;
				$w['ismust'] = 0;
				$w['isshow'] = 1;
				...
				$w['field'] = 'keywords';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('关键词');
				$w['tips'] = NEXTLANG('每个词用英文逗号(,)拼接');
				$w['fieldtype'] = 1;
				$w['fieldlong'] = 255;
				$w['ismust'] = 0;
				$w['isshow'] = 1;
				...
				$w['field'] = 'member_id';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('发布会员');
				$w['tips'] = NEXTLANG('前台发布会员ID记录');
				$w['fieldtype'] = 13;
				$w['fieldlong'] = 11;
				$w['body'] = '3,username';
				$w['ismust'] = 0;
				...
				$w['field'] = 'userid';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('管理员');
				$w['tips'] = NEXTLANG('后台发布管理员ID记录');
				$w['fieldtype'] = 13;
				$w['fieldlong'] = 11;
				$w['body'] = '11,name';
				$w['ismust'] = 0;
				...
				$w['field'] = 'target';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('外链URL');
				$w['tips'] = NEXTLANG('默认为空，系统访问内容则直接跳转到此链接');
				$w['fieldtype'] = 1;
				$w['fieldlong'] = 255;
				$w['ismust'] = 0;
				$w['isshow'] = 0;
				...
				$w['field'] = 'ownurl';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('自定义URL');
				$w['tips'] = NEXTLANG('默认为空，自定义URL');
				$w['fieldtype'] = 1;
				$w['fieldlong'] = 255;
				$w['ismust'] = 0;
				$w['isshow'] = 0;
				...
				$w['field'] = 'hits';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('点击量');
				$w['tips'] = NEXTLANG('系统自动添加');
				$w['fieldtype'] = 4;
				$w['fieldlong'] = 11;
				$w['body'] = '';
				$w['ismust'] = 0;
				...
				$w['field'] = 'comment_num';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('评论数');
				$w['tips'] = NEXTLANG('系统自带');
				$w['fieldtype'] = 4;
				$w['fieldlong'] = 11;
				$w['body'] = '';
				$w['ismust'] = 0;
				...
				$w['field'] = 'addtime';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('添加时间');
				$w['tips'] = NEXTLANG('选择时间');
				$w['fieldtype'] = 11;
				$w['fieldlong'] = 11;
				$w['format'] = 'date_2';
				$w['body'] = '';
				...
				$w['field'] = 'nextattr';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('推荐属性');
				$w['tips'] = NEXTLANG('1置顶2热点3推荐');
				$w['fieldtype'] = 16;
				$w['fieldlong'] = 50;
				$w['format'] = NULL;
				$w['body'] = $id.',name';
				...
				$w['field'] = 'isshow';
				$w['molds'] = $data['biaoshi'];
				$w['fieldname'] = NEXTLANG('是否显示');
				$w['tips'] = NEXTLANG('显示隐藏');
				$w['fieldtype'] = 7;
				$w['fieldlong'] = 1;
				$w['format'] = NULL;
				$w['body'] = NEXTLANG('显示=1,未审=0,退回=2');
				$w['ismust'] = 0;
				$w['isshow'] = 1;
				$w['isadmin'] = 1;
				$w['issearch'] = 1;
				...
				if(strlen($data['name'])>12){
					...
					$ruler['name'] = $data['name'].NEXTLANG('列表');
				}
				...
				if($data['ismust']==1 && $data['isshowclass']==1){
					JsonReturn(array('code'=>0,'msg'=>NEXTLANG('新增模型成功，快去创建对应的栏目吧！'),'url'=>U('Classtype/addclass',['biaoshi'=>$data['biaoshi']])));
				}else{
					JsonReturn(array('code'=>0,'msg'=>NEXTLANG('新增模型成功，快去设置表字段吧！'),'url'=>U('Fields/index',['molds'=>$data['biaoshi']])));
				}
				
			
				
			}else{
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('新增模型失败！')));
	...
	function editMolds(){
		...
		if($this->frparam('go',1)==1){
			...
			if($this->frparam('id')){
				if(!$data['biaoshi']){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('标识不能为空！')));
					exit;
				}
				$molds = $this->data;
				if($molds['sys']==1 && $data['biaoshi']!=$molds['biaoshi']){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('系统模型标识不允许修改！')));
					exit;
				}
				...
				if(M('Molds')->update(array('id'=>$this->frparam('id')),$data)){
					...
					if($this->data['biaoshi']!=$data['biaoshi']){
						...
						$x = M()->runSql($sql);
						JsonReturn(array('code'=>0,'msg'=>NEXTLANG('修改成功！'),'url'=>U('index')));
					...
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('修改失败！')));
					
				}
			}else{
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('页面有错误，缺少模块ID！')));
			}
	...
	function deleteMolds(){
		...
		if($id){
			...
			if($nums){
				JsonReturn(array('code'=>1,'msg'=>$molds['name'].NEXTLANG('里面存在数据，请先清空表内数据！')));
			}
			if($molds['sys']==1){
				JsonReturn(array('code'=>1,'msg'=>$molds['name'].NEXTLANG('是系统模型，不允许删除！')));
			}
			if(M('Molds')->delete('id='.$id)){
				...
				if(M('Fields')->find(array('molds'=>$molds['biaoshi']))){
					...
					if(!$re){
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段表记录未清除，请手动清除！')));
					}
				}
	...
	function restrucFields(){
        ...
        if($_POST){
            ...
            foreach ($title as $k=>$v){
                if(!$v){
                    JsonReturn(['code'=>1,'msg'=>$field[$k]].NEXTLANG('字段名称不能为空！'));
                }
                $w = [];
                $w['molds'] = $molds;
                $w['field'] = $field[$k];
            ...
            if(count($newfields)){
                foreach ($newfields as $v){
                    ...
                    switch ($w['field']){
                        case 'isshow':
                            $w['body'] = NEXTLANG('显示=1,隐藏=0,退回=2');
                            break;
                        case 'islist':
                        case 'isadmin':
                            $w['body'] = NEXTLANG('显示=1,隐藏=0');
            ...
            JsonReturn(['code'=>0,'msg'=>NEXTLANG('重构成功！'),'url'=>U('Fields/index',['molds'=>$molds])]);
