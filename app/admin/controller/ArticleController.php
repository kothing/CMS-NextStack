	...
	function addarticle(){
		...
		if($this->frparam('go',1)==1){
			...
			if($r){
				...
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('添加成功,继续添加~'),'url'=>U('addarticle',array('tid'=>$data['tid']))));
				exit;
			}
		...
		if(!$config){
			...
			M('sysconfig')->add(['title'=>NEXTLANG('内容配置'),'field'=>'article_config','type'=>3,'data'=>json_encode($configdata,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),'typeid'=>0]);
			setCache('webconfig',null);
		}else{
			$configdata = json_decode($config,1);
		}
			...
			if($this->webconf['mingan'] && $this->frparam('config_filter',1)){
				...
				foreach($mingan as $s){
					if(strpos($s,'{xxx}')!==false){
						...
						foreach($filter as $vv){
							if($vv && preg_match($pattern, $data[$vv])){
								JsonReturn(array('code'=>1,'msg'=>NEXTLANG('修改失败').'，【'.$fields[$vv].'】'.NEXTLANG('存在敏感词').' [ '.$s.' ]'));
							}
				...
				if(M('Article')->update(array('id'=>$this->frparam('id')),$data)){
					...
					if($this->webconf['release_award_open']==1 && $data['isshow']==1){
						...
						if($member_id!=0 && $award>0){
							$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>'article','aid'=>$this->frparam('id'),'msg'=>NEXTLANG('发布奖励')]);
							if(!$rr){
				...
				foreach ($all as $k => $v) {
					if($v['isshow']!=1){
						...
						if($this->webconf['release_award_open']==1){
							...
							if($member_id!=0 && $award>0){
								$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>'article','aid'=>$v['id'],'msg'=>NEXTLANG('发布奖励')]);
								if(!$rr){
