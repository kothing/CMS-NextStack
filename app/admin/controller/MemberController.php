...
class MemberController extends CommonController
{
    ...
    function memberadd(){
        ...
        if($this->frparam('go')==1){
            ...
            if(M('member')->find(['email'=>$data['email']])){
                JsonReturn(array('code'=>1,'msg'=>NEXTLANG('邮箱已被注册！')));
            }
            ...
            if(M('member')->add($data)){
                JsonReturn(array('code'=>0,'msg'=>NEXTLANG('添加成功！')));
            }else{
                JsonReturn(array('code'=>1,'msg'=>NEXTLANG('添加失败，请重新提交！')));
            }
	...
	function group_del(){
		...
		if($id){
			...
			if(M('member')->getCount(array('gid'=>$id))>0){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('该分组下存在用户，请先移除用户再删除！')));
			}
	...
	public function editrulers(){
		...
		if($this->frparam('go')==1){
			...
			if($a){
				...
				JsonReturn(array('status'=>1,'info'=>NEXTLANG('修改失败！')));
			}
		}
	...
	public function deleterulers(){
		...
		if($id){
			...
			if($n){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('删除失败，该分类有下级功能，请先删除下级功能！')));
			}else{
			...
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('未选择删除对象！')));
		}
	}
