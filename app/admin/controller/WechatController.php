...
class WechatController extends CommonController
{
	 function wxcaidan(){
		if($this->frparam('go')==1){
			...
			if($re['errcode']==0){
				Success(NEXTLANG('更新成功！'),U('wxcaidan'));
			}else{
		...
		if(strpos($weixincaidan_json,'errcode')!==false){
			...
			$a2 = array('type'=>"view","name"=>NEXTLANG('个人中心'),'url'=>get_domain().'/login.html',"sub_button"=>[]
			);
			$a3 =  array('type'=>"","name"=>NEXTLANG('测试1'),'url'=>'',"sub_button"=>[
				['type'=>'view','name'=>NEXTLANG('测试2'),'url'=>'xxxx'],
				['type'=>'view','name'=>NEXTLANG('测试3'),'url'=>'xxxx']]
			);
	...
	public function getAccessToken(){
		...
		if(isset($obj['errcode'])){
			exit(NEXTLANG('微信配置错误！').$obj['errcode'].$obj['errmsg']);
		}
