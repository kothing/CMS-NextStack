...
class SysController extends CommonController
{
        ...
        if($_POST){
            ...
            if($this->frparam('deltype')){
                ...
                if($ctype){
                    ...
                    if($isok){
                        JsonReturn(['code'=>1,'msg'=>NEXTLANG('配置栏下有自定义配置，无法删除配置栏')]);
                    }
                ...
                JsonReturn(['code'=>1,'msg'=>NEXTLANG('操作失败！')]);
            }
            $whereArr = [];
            $valueArr = [];
            ...
            if($this->frparam('custom_title',1) && $this->frparam('custom_type')){
                ...
                if($this->frparam('custom_fields',1)){
                    ...
                    $n = M('sysconfig')->add($new);
                    if($n){
                        M('sysconfig')->update(['id'=>$n],['field'=>'next_'.$n]);
                    }
                }
            }
            if($this->frparam('custom_new_title',1) && $this->frparam('custom_new_fields',1)){
                ...
                if(M('ctype')->find($ww)){
                    JsonReturn(['code'=>1,'msg'=>NEXTLANG('已存在配置标识，请重新设置！')]);
                }
            ...
            JsonReturn(['code'=>0,'msg'=>NEXTLANG('提交成功！')]);
    ...
    public function custom_del(){
	   ...
	   if($field){
			...
			if($r){
				setCache('webconfig',null);
				JsonReturn(['code'=>0,'msg'=>NEXTLANG('删除成功！')]);
			}else{
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('删除失败！系统字段不允许删除！')]);
			}
		    
	   }
	    JsonReturn(['code'=>1,'msg'=>NEXTLANG('删除失败！')]);
		...
		if($id){
			...
			if(strpos($pic['litpic'],'http')===false){
				if(file_exists('.'.$pic['litpic'])){
					...
					JsonReturn(['code'=>1,'msg'=>NEXTLANG('图片不存在，删除失败！')]);
				}
		...
		if ($_FILES["file"]["error"] > 0){
		  ...
		  // echo "Stored in: " . $_FILES["file"]["tmp_name"];
			...
			if(move_uploaded_file($_FILES["file"]['tmp_name'],$filename)){
				...
				$data['error'] =  "Error: ".NEXTLANG("请检查目录")."[static/upload/cert]".NEXTLANG("写入权限");
				$data['code'] = 1001;
	...
	public function addcache(){
		if($this->frparam('go',1)==1){
			...
			if(M('cachedata')->add($data)){
				...
				$time = $data['times']*60;
				setCache('nextcache_'.$data['field'],$result,$time);
				
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('添加成功！继续添加~'),'url'=>U('sys/addcache')));
				exit;
			}else{
	...
	public function editcache(){
		...
		if($this->frparam('go',1)==1){
			...
			if(M('cachedata')->update(['id'=>$id],$data)){
				...
				$time = $data['times']*60;
				setCache('nextcache_'.$data['field'],$result,$time);
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('修改成功！'),'url'=>U('sys/datacache')));
				exit;
			}else{
    ...
    public function editctype(){
        ...
        if(!$id){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('链接错误！')));
        }
        if(!$title || !$action){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('配置栏名称和配置标识不能为空！')));
        }
        $r = M('ctype')->find("id!=".$id." and title='".$title."'");
        if($r){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('配置栏名称已存在！')));
        }
        $r = M('ctype')->find("id!=".$id." and action='".$action."'");
        if($r){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('配置标识已存在！')));
        }
    ...
    public function ctype(){
        ...
        if(!$ctype){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('没有找打相关配置！')));
        }
    ...
    public function setfield(){
        ...
        if(!$id || !$field || !$title || !$type || !$typeid){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('参数错误！')));
        }
        $r = M('sysconfig')->find("id!=".$id." and field='".$field."'");
        if($r){
            JsonReturn(array('code'=>1,'msg'=>NEXTLANG('配置字段已存在！')));
        }
