...
class ProductController extends CommonController
{
	...
	function addproduct(){
		...
		if($this->frparam('go',1)==1){
					...
					if(strpos($s,'{xxx}')!==false){
						...
						foreach($filter as $vv){
							if($vv && preg_match($pattern, $data[$vv])){
								JsonReturn(array('code'=>1,'msg'=>NEXTLANG('添加失败').'，【'.$fields[$vv].'】'.NEXTLANG('存在敏感词').' [ '.$s.' ]'));
							}
            ...
            if($this->webconf['hidetitleonliy']){
                ...
                if($onliyfield){
                    if(M('product')->find([$onliyfield=>$data[$onliyfield]])){
                        JsonReturn(array('code'=>1,'msg'=>$onliyfield.NEXTLANG('重复！')));
                    }
                }
            }
			...
			if($r){
				...
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('添加成功,继续添加~'),'url'=>U('addproduct',array('tid'=>$data['tid']))));
				exit;
			}
		...
		if(!$config){
			...
			M('sysconfig')->add(['title'=>NEXTLANG('商品配置'),'field'=>'product_config','type'=>3,'data'=>json_encode($configdata,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),'typeid'=>0]);
			setCache('webconfig',null);
		}else{
			$configdata = json_decode($config,1);
		}
	...
	function editproduct(){
				...
				if(M('product')->update(array('id'=>$this->frparam('id')),$data)){
					...
					if($this->webconf['release_award_open']==1 && $data['isshow']==1){
						...
						if($member_id!=0 && $award>0){
							$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>'product','aid'=>$this->frparam('id'),'msg'=>NEXTLANG('发布奖励')]);
							if(!$rr){
								$start = mktime(0, 0, 0, date('m'), date('d'), date('Y'));
								$end = mktime(23, 59, 59, date('m'), date('d'), date('Y'));

								$sql = " addtime>=".$start." and addtime<".$end." and userid=".$member_id." and type=3 and msg='".NEXTLANG("发布奖励")."' ";
					...
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('您未做任何修改，不能提交！')));
					exit;
				}
			}
		...
		if(!$config){
			...
			M('sysconfig')->add(['title'=>NEXTLANG('内容配置'),'field'=>'product_config','type'=>3,'data'=>json_encode($configdata,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),'typeid'=>0]);
			setCache('webconfig',null);
		}else{
			$configdata = json_decode($config,1);
		}
	...
	function deleteproduct(){
		...
		if($id){
			...
			if(M('product')->delete(['id'=>$id])){
				...
				if($customurl){
					$w['molds'] = 'customurl';
					$w['title'] = '['.$customurl['id'].']'.NEXTLANG('自定义链接');
	...
	function copyproduct(){
		...
		if($id){
			...
			if(M('product')->add($data)){
				...
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('复制成功！')));
				exit;
			}else{
				
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('复制失败！')));
				exit;
			}
			...
			if(M('product')->delete('id in('.$data.')')){
				...
				foreach($all as $v){
					...
					if($x && $newcustomurl[$v['id']]){
						$w['molds'] = 'customurl';
						$w['title'] = '['.$newcustomurl['id'].']'.NEXTLANG('自定义链接');
	...
	function copyAll(){
		...
		if($data!=''){
			...
			if($r){
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('批量复制成功！')));
				
			}else{
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('批量复制失败！')));
			}
		}
	}
	//批量修改
	function changeType(){
		...
		if($data!=''){
			...
			JsonReturn(array('code'=>0,'msg'=>NEXTLANG('批量修改成功！')));
		}
	}
	//修改排序
	...
	function changeAttribute(){
		...
		if($data!=''){
			...
			$r = true;
			foreach($list as $v){
				if(strpos($v['nextattr'],','.$tj.',')!==false){
					$attr = str_replace(','.$tj.',','',$v['nextattr']);
					if(!$attr){
						$w['nextattr'] = '';
					}else{
						$w['nextattr'] = ','.trim($attr,',').',';
					}
				}else{
					if($v['nextattr']){
						$w['nextattr'] = $v['nextattr'].$tj.',';
					}else{
						$w['nextattr'] = ','.$tj.',';
					}
				}
	...
	function checkAll(){
		...
		if($data!=''){
				...
				foreach ($all as $k => $v) {
					if($v['isshow']!=1){
						...
						if($this->webconf['release_award_open']==1){
							...
							if($member_id!=0 && $award>0){
								$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>'product','aid'=>$v['id'],'msg'=>NEXTLANG('发布奖励')]);
								if(!$rr){
			...
			JsonReturn(array('code'=>0,'msg'=>NEXTLANG('批量操作成功！')));
		}else{
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('批量操作失败！')));
		}
	}
