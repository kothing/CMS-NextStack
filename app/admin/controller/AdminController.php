...
class AdminController extends CommonController
{
	...
	function group_del(){
		...
		if($id){
			...
			if(M('level')->getCount(array('gid'=>$id))>0){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('该角色下存在用户，请先移除用户再删除！')));
			}
			if($id==1){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('删除失败，该分组不允许删除！')));
			}
			...
			if(M('level_group')->delete(array('id'=>$id))){
				...
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('删除失败，请重试！')));
			}
		}else{
	...
	function groupedit(){
		...
		if($this->frparam('go')==1){
			...
			if($this->admin['gid']!=1 && $this->frparam('isadmin')==1){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('修改失败，您的权限不足！')));
			}
	...
	function groupadd(){
		...
		if($this->frparam('go')==1){
			$data = $this->frparam();
            if($this->admin['gid']!=1 && $this->frparam('isadmin')==1){
                JsonReturn(array('code'=>1,'msg'=>NEXTLANG('您的权限不足！')));
            }
			$data['name'] = $this->frparam('name',1);
			$data['ischeck'] = $this->frparam('ischeck');
			$data['description'] = $this->frparam('description',1);
	...
	public function change_group_status(){
		...
		if($id==1){
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('修改失败，该分组不允许修改！')));
		}
	...
	public function admindelete(){
		...
		if($id==1){
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('系统管理员不能删除！')));
		}
