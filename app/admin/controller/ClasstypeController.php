...
class ClasstypeController extends CommonController
{
	...
	function addclass(){
		...
		if($this->frparam('go')==1){
			...
			if(in_array(strtolower($htmlurl),array('message','user','comment','home','common','order','tags','wechat','login'))){
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('URL链接命名不能是').'：message,user,comment,home,common,order,tags,wechat,login,nextpay'));
			}
			if(stripos($htmlurl,'.php')!==false){
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('非法URL')));
			}
			...
			if($a){
				...
				JsonReturn(array('status'=>1,'info'=>NEXTLANG('添加栏目成功，继续添加~'),'url'=>U('addclass',array('pid'=>$w['pid'],'biaoshi'=>$w['molds']))));
			}else{
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('新增失败！')));
			}
		}
		//模块
	...
	function editclass(){
		...
		if($this->frparam('go')==1){
			...
			if(!M('molds')->find(['biaoshi'=>$w['molds']])){
				JsonReturn(array('status'=>1,'info'=>NEXTLANG('模型错误！')));
			}
			$w['description'] = $this->frparam('description',1);
			$w['keywords'] = $this->frparam('keywords',1);
			...
			if(checkClass($w['pid'],$this->data['id']) || ($w['pid']==$this->data['id'])){
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('不能选择当前栏目及下级为顶级栏目')));
			}
			...
			if($a){
				...
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('您未做任何修改，不能提交！')));
			}
		}
	...
	function deleteclass(){
		...
		if($id){
			...
			if(M('classtype')->find(['pid'=>$id])){
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('该栏目有子栏目，请先删除子栏目！')));
			}
			...
			if($a){
				...
				JsonReturn(array('status'=>0,'info'=>NEXTLANG('删除失败！')));
			}
		}
	...
	function addmany(){
		if($_POST){
			...
			$classname = $this->frparam('classname',1);
			if(!trim($classname)){
				JsonReturn(['code'=>1,'msg'=>NEXTLANG('栏目不能为空！')]);
			}
			$classname = explode("\n",trim($classname));
			$classtypetree = classTypeData();
	...
	function changeClass(){
		...
		foreach($tids_arr as $v){
			...
			if(checkClass($pid,$v) || ($pid==$v)){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('不能选择当前栏目及下级为顶级栏目')));
			}
		}
