...
class FieldsController extends CommonController
{
	...
	function index(){
		if($this->frparam('molds',1)==''){
			Error(NEXTLANG('请选择模块！'));
		}
		if($this->frparam('ajax')){

			$data = M('fields')->findAll(array('molds'=>$this->frparam('molds',1)),'orders desc');
			foreach($data as &$v){
				$v['isadmin'] = $v['isadmin']==1 ? NEXTLANG('显示') : NEXTLANG('隐藏');
				$v['isshow'] = $v['isshow']==1 ? NEXTLANG('显示') : NEXTLANG('隐藏');
				$v['ishome'] = $v['ishome']==1 ? NEXTLANG('显示') : NEXTLANG('隐藏');
				$v['islist'] = $v['islist']==1 ? NEXTLANG('显示') : NEXTLANG('隐藏');
				$v['issearch'] = $v['issearch']==1 ? NEXTLANG('显示') : NEXTLANG('隐藏');
				$v['ismust'] = $v['ismust']==1 ? NEXTLANG('是') : NEXTLANG('否');
				$v['isext'] = $v['isext']==1 ? NEXTLANG('是') : NEXTLANG('否');

				switch($v['fieldtype']){
					case 1:
					$v['fieldtypename'] = NEXTLANG('单行文本');
					break;
					case 2:
					$v['fieldtypename'] = NEXTLANG('多行文本');
					break;
					case 3:
					$v['fieldtypename'] = NEXTLANG('文本编辑器');
					break;
					case 4:
					$v['fieldtypename'] = NEXTLANG('数字');
					break;
					case 5:
					$v['fieldtypename'] = NEXTLANG('单图片');
					break;
					case 6:
					$v['fieldtypename'] = NEXTLANG('多图片');
					break;
					case 7:
					$v['fieldtypename'] = NEXTLANG('单选下拉');
					break;
					case 8:
					$v['fieldtypename'] = NEXTLANG('多选');
					break;
					case 9:
					$v['fieldtypename'] = NEXTLANG('单附件');
					break;
					case 10:
					$v['fieldtypename'] = NEXTLANG('多附件');
					break;
					case 11:
					$v['fieldtypename'] = NEXTLANG('时间戳');
					break;
					case 12:
					$v['fieldtypename'] = NEXTLANG('单选按钮');
					break;
					case 13:
					$v['fieldtypename'] = NEXTLANG('单选关联');
					break;
					case 14:
					$v['fieldtypename'] = NEXTLANG('小数');
					break;
					case 15:
					$v['fieldtypename'] = NEXTLANG('多行录入');
					break;
					case 16:
					$v['fieldtypename'] = NEXTLANG('多选关联');
					break;
                    case 17:
                    $v['fieldtypename'] = NEXTLANG('栏目');
                    break;
                    case 18:
                    $v['fieldtypename'] = NEXTLANG('副栏目');
                    break;
					case 19:
                    $v['fieldtypename'] = NEXTLANG('系统TAG');
                    break;
				}
	...
	function addFields(){
		...
		if($this->frparam('go',1)==1){
			...
			if($data['fieldname']=='' || $data['field']==''){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段名和字段标识不能为空！')));
			}
			
			//检测是否存在该模块
			if(M('Fields')->find(array('field'=>$data['field'],'molds'=>$data['molds']))){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段标识已存在！')));
			}
			//取消保护字段，可以继续创建
			if(in_array($data['field'],['sql','nextcache','nextcachetime','table','orderby','limit','ispage','notin','in','empty','notempty','fields','like','day','as','file'])){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('系统保护字段，不允许创建！')));
			}
			...
			switch($data['fieldtype']){
				...
				if($data['fieldlong']>11 || $data['fieldlong']<=0){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段长度不对！')));
				}
				...
				if($data['fieldlong']!=11){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段长度不对,时间属性必须长度为11')));
				}
				$sql .= "INT(".$data['fieldlong'].") DEFAULT ";
				if($data['vdata']){
					$sql .=  "'".$data['vdata']."'";
				...
				if(strpos($data['fieldlong'],',')===false){
					JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段长度不对,decimal字段长度格式[整数位数,小数位数]')));
				}
			...
			if(!$n){
				...
				//M()->runSql($delsql);
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段创建成功，但是字段表记录失败，请反馈官方解决！')));
			}
			JsonReturn(array('code'=>0,'msg'=>NEXTLANG('字段创建成功！')));
			...
			if($this->frparam('id')){
				...
				if($old['field']==$data['field']){
					...
					if(M('Fields')->update(array('id'=>$this->frparam('id')),$data)){
						JsonReturn(array('code'=>0,'msg'=>NEXTLANG('字段修改成功！')));
					}else{
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('字段修改失败！')));
					}
					
				}else{
					if(in_array($data['field'],['id','sql','nextcache','nextcachetime','table','orderby','limit','ispage','notin','in','empty','notempty','fields','like','tids','day','as','istop','istuijian','ishot','isall','file'])){
						JsonReturn(array('code'=>1,'msg'=>NEXTLANG('系统保护字段，不允许创建！')));
					}
				}
		...
		foreach($fields_list as $k=>$v){
			...
			switch($v['fieldtype']){
                ...
                $l .= $v['fieldname'].'  
					...
					<div class="layui-input-inline">
						<input name="'.$v['field'].'" placeholder="'.NEXTLANG('上传图片').'" type="text" class="layui-input" id="'.$v['field'].'" ';
					if($v['ismust']==1){
						$l.=' required="" lay-verify="required" ';
					}
				  ...
				  <div class="layui-field-box">
					  <div class="layui-input-block">
						  <div class="site-demo-upbar">
							<button type="button" class="layui-btn" id="LAY_'.$v['field'].'_upload">
							  <i class="layui-icon">&#xe67c;</i>'.NEXTLANG('上传图片').'
							</button>
							 '.$v['tips'].'
						  </div>
					...
					if($data[$v['field']]!=''){
						foreach(explode('||',$data[$v['field']]) as $vv){
                            $pic = explode('|',$vv);
                            $l.='<div class="upload-icon-img layui-input-inline" ><div class="upload-pre-item"><img src="'.$pic[0].'" class="img" width="200px" height="200px" ><input name="'.$v['field'].'_urls[]" type="text" class="layui-input"  value="'.$pic[0].'" /><input name="'.$v['field'].'_des[]" type="text" class="layui-input" placeholder="'.NEXTLANG('文字描述').'"  value="'.$pic[1].'" /><a class="layui-btn layui-btn-sm layui-btn-radius layui-btn-danger delete_file">'.NEXTLANG('删除').'</a><a class="layui-btn layui-btn-sm layui-btn-radius imgorder " onclick="goleft(this)">'.NEXTLANG('左移').'</a><a class="layui-btn layui-btn-sm layui-btn-radius imgorder " onclick="goright(this)">'.NEXTLANG('右移').'</a></div></div>';
                ...
                $l .= $v['fieldname'].'  
                    ...
                    <div class="layui-input-inline">
						<select name="'.$v['field'].'" lay-search="" id="'.$v['field'].'" ><option value="">'.NEXTLANG('请选择').'</option>';
				...
				$l .= '<fieldset class="layui-elem-field">
				  ...
				  <div class="layui-field-box">
					  <div class="layui-input-block">
						  <div class="site-demo-upbar">
							<button type="button" class="layui-btn" id="LAY_'.$v['field'].'_upload">
							  <i class="layui-icon">&#xe67c;</i>'.NEXTLANG('上传附件').'
							</button>
							 '.$v['tips'].'
						  </div>
...
layui.use("laydate", function(){
  ...
  laydate'.$randt.'.render({elem: "#laydate_'.$v['field'].'",type:"datetime",trigger: "click" });});</script>';
				...
				if(!$biaoshi){
					echo $v['field'].NEXTLANG('字段关联绑定失败，请重新绑定！');exit;
				}
				$datalist = M($biaoshi)->findAll();
				$l.='<option value="0">'.NEXTLANG('请选择关联项').'</option>';
				...
				if($data[$v['field']]){
					...
					foreach($rs as $vv){
						$l.='<div class="layui-input-block"><input type="text"  style="width:500px;" value="'.$vv.'" name="'.$v['field'].'[]" autocomplete="off" class="layui-input layui-input-inline"><button type="button" class="layui-btn layui-btn-danger layui-btn-sm  layui-input-inline '.$v['field'].'_del" >'.NEXTLANG('删除').'</button></div>';
					}
				}else{
				...
				$(document).ready(function(){
					$("#'.$v['field'].'_add").click(function(){
						var html = \'<div class="layui-input-block"><input type="text"  style="width:500px;" value="" name="'.$v['field'].'[]" autocomplete="off" class="layui-input layui-input-inline"><button type="button" class="layui-btn layui-btn-danger layui-btn-sm  layui-input-inline '.$v['field'].'_del" >'.NEXTLANG('删除').'</button></div>\';
        	   ...
        	   $l.=' ])
				...
				$l.='<div class="layui-form-item layui-form-text">
                    <label for="'.$v['field'].'" class="layui-form-label">
                        '.NEXTLANG('TAG标签').' [ '.NEXTLANG('按Enter回车自动添加').' ]
                    </label>
                    <div class="layui-input-block">
  ...
  laydate'.$randt.'.render({elem: "#laydate_'.$v['field'].'",type:"datetime",trigger: "click",range:"~" });});</script>';
	...
	function deleteFields(){
		...
		if($id){
			...
			//不允许删除字段
			$noallow = ['addtime','member_id','hits','target','ownurl','id','molds','htmlurl','nextattr','tids','tid','litpic','title','keywords','seo_title','body'];
			if(in_array($fields['field'],$noallow)){
				JsonReturn(array('code'=>1,'msg'=>NEXTLANG('系统字段不允许删除！')));
			}
	...
	function changeTid(){
		...
		if(!$ids){
			JsonReturn(['code'=>1,'msg'=>NEXTLANG('请选择字段！')]);
		}
		$tid = $this->frparam('tid');
		if(!$tid){
			JsonReturn(['code'=>1,'msg'=>NEXTLANG('请选择栏目！')]);
		}
		$sql = 'id in('.$ids.') ';
	...
	function changeFieldList(){
		...
		if(in_array($field,['issearch','islist','width','listorders'])){
			...
			JsonReturn(array('code'=>1,'info'=>NEXTLANG('非法操作！')));
		}
