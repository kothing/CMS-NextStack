...
class ExtmoldsController extends Controller
{
	...
	public function addmolds(){
		...
		if($this->frparam('go',1)==1){
			...
			if($r){
				...
				JsonReturn(array('code'=>0,'msg'=>NEXTLANG('添加成功,继续添加~'),'url'=>U('Extmolds/addmolds',['tid'=>$data['tid'],'molds'=>$molds])));
				...
				if(M($molds)->update(array('id'=>$this->frparam('id')),$data)){
					...
					if($this->webconf['release_award_open']==1 && $data['isshow']==1){
						...
						if($member_id!=0 && $award>0){
							$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>$molds,'aid'=>$this->frparam('id'),'msg'=>NEXTLANG('发布奖励')]);
							if(!$rr){
	...
	function checkAll(){
		...
		if($data!=''){
				...
				foreach ($all as $k => $v) {
					if($v['isshow']!=1){
						...
						if($this->webconf['release_award_open']==1){
							...
							if($member_id!=0 && $award>0){
								$rr = M('buylog')->find(['userid'=>$member_id,'type'=>3,'molds'=>$molds,'aid'=>$v['id'],'msg'=>NEXTLANG('发布奖励')]);
								if(!$rr){
			...
			JsonReturn(array('code'=>0,'msg'=>NEXTLANG('批量审核成功！')));
		}else{
			JsonReturn(array('code'=>1,'msg'=>NEXTLANG('批量审核失败！')));
		}
	}
}