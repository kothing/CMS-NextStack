...
//输出指定字段的标题和内容
function next_show_fields($data=array(),$fields=null){
	$sql = array();
	if($data['tid']!=0){
...
function send_mail($send_mail,$password,$send_name,$to_mail,$title,$body,$email_ext=''){
	...
	require_once(APP_PATH.'framework/extend/PHPMailer/PHPMailerAutoload.php');
 ...
 */
function nextresize($src_image,$out_width = NULL, $out_height = NULL, $mode = 1, $out_image = NULL,  $direct = 1 ,$debug = 0 , $img_quality = 90 ) {
...
function nextcachedata($field){
	$result = getCache('nextcache_'.$field);
	if(!$result){
		...
		$time = $res['times']*60;
		setCache('nextcache_'.$res['field'],$result,$time);
	}
	return $result;
}
// 增加classtypedata缓存
